...
nb_clf = Pipeline([('vectorizer', TfidfVectorizer(stop_words=nltk.corpus.stopwords.words('english'), tokenizer=tokenize)),
                   ('classifier', MLPClassifier(max_iter=300, hidden_layer_sizes=50, shuffle=True, activation='relu', solver='lbfgs'))])
...
confusion_matrix_df.to_csv("out_nn.csv", sep='\t', encoding='utf-8')